...
namespace clins
{
  ...
  void LidarOdometry<_N>::LoopClosureHandler()
  {
    ...
    *cloud_key_pos_xy_copied_ = *cloud_key_pos_xy_;
    if (cloud_key_pos_xy_copied_->points.size() == 0)
      return;
    //  loop_mutex_.unlock();
  ...
  const bool LidarOdometry<_N>::isKeyFrame(double time) const
  {
    ...
    if ((time - cloud_key_pos_->back().timestamp) > 10)
      return true;
                                            ...
                                            double *max_time)
  {
    ...
    for (auto const &v : input_cloud->points)
    {
      ...
      if (max_time && (*max_time < p.timestamp))
        *max_time = p.timestamp;
    }
    // Resize to the correct size
    if (cnt != input_cloud->size())
      output_cloud->resize(cnt);
      ...
      const LiDARFeature &feature_cur, const LiDARFeature &feature_cur_in_M)
  {
    ...
    {
      if (feature_cur.surface_features->points[i].timestamp < 0)
        continue;
  ...
  bool LidarOdometry<_N>::PointToPointICP(GPointCloud::Ptr target_cloud,
                                          ...
                                          float &fitness_score)
  {
    if (correspondence_distance < 1)
      correspondence_distance = 1;
